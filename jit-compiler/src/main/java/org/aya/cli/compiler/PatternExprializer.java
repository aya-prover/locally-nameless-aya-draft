...
import static org.aya.cli.compiler.PatternSerializer.*;
  ...
  public AyaSerializer<Pat> serialize(Pat unit) {
    switch (unit) {
      ...
      case Pat.Bind bind -> {
        ...
        builder.append(STR."new \{CLASS_PAT_BIND}(new LocalVar(\"dogfood\"), ErrorTerm.DUMMY)");
      }
      case Pat.ConLike con -> {
        var instance = PatternSerializer.getQualified(con);

        builder.append(STR."new \{CLASS_PAT_JCON}(\{getInstance(instance)}, ");
        serialize(con.args());
        builder.append(")");
      }
